...
description: >
  ...
  coverage. Do not edit source. Spawned by /implement-stage.
prompt_mode: full
model: inherit
...
Write the verdict using the schema path the orchestrator put in the prompt (implement-stage `references/verdict.md`). `gate` is `test-review`.
